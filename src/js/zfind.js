(function($) {
    $.fn.findPost = function(options) {
        var settings = $.extend({
            jsonFile        : '/search.json',
            jsonFormat      : 'title,url,date',
            template        : '<li><article><a href="{url}">{title}<br><span class="date">{date}</span></a></article></li>',
            searchResults   : '.search-results',
            limit           : '5',
            noResults       : '<p>Sem resultados...<br><small>provavelmente o que você procura esta além do nosso conhecimento :P</small></p>'
        }, options);
